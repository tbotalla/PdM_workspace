...
__STATIC_INLINE void __NVIC_SetPriority(IRQn_Type IRQn, uint32_t priority) {
    if ((int32_t)(IRQn) >= 0) {
        ...
        SCB->SHPR[(((uint32_t) IRQn) & 0xFUL) - 4UL] = (uint8_t)(
...
__NO_RETURN __STATIC_INLINE void __SW_SystemReset(void) {
    ...
    SCB->AIRCR = (uint32_t)((0x5FAUL << SCB_AIRCR_VECTKEY_Pos) |
                            ...
                            /* Keep BFHFNMINS unchanged. Use this Reset function in case your case need to keep it */
...
__STATIC_INLINE void TZ_NVIC_SetPriority_NS(IRQn_Type IRQn, uint32_t priority) {
    if ((int32_t)(IRQn) >= 0) {
        ...
        SCB_NS->SHPR[(((uint32_t) IRQn) & 0xFUL) - 4UL] = (uint8_t)(
            (priority << (8U - __NVIC_PRIO_BITS)) & (uint32_t) 0xFFUL);
    }
}
